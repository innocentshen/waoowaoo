...
export function useProjectData(projectId: string | null) {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.ok) {
                throw new Error(await readApiErrorMessage(res, `Failed to load project (HTTP ${res.status})`))
            }
...
export function useRefreshProjectData(projectId: string | null) {
    ...
    return () => {
        if (projectId) {
            return queryClient.invalidateQueries({ queryKey: queryKeys.projectData(projectId) })
        }
...
export function useEpisodeData(projectId: string | null, episodeId: string | null) {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.ok) {
                throw new Error(await readApiErrorMessage(res, `Failed to load episode (HTTP ${res.status})`))
            }
...
export function useRefreshAll(projectId: string | null, episodeId: string | null) {
    ...
    return () => {
        ...
        if (projectId) {
            invalidations.push(queryClient.invalidateQueries({ queryKey: queryKeys.projectData(projectId) }))
            invalidations.push(queryClient.invalidateQueries({ queryKey: queryKeys.projectAssets.all(projectId) }))
        }
        if (projectId && episodeId) {
