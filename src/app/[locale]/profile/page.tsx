...
import PromptCenterTab from './components/PromptCenterTab'
...
export default function ProfilePage() {
  ...
  // 主要分区：扣费记录 / API配置
  const [activeSection, setActiveSection] = useState<'billing' | 'apiConfig' | 'promptCenter'>('apiConfig')
          ...
          <div className="w-64 flex-shrink-0">
            <div className="glass-surface-elevated h-full flex flex-col p-5">
              ...
              <nav className="flex-1 space-y-2">
                ...
                <button
                  onClick={() => setActiveSection('promptCenter')}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all cursor-pointer ${activeSection === 'promptCenter'
                    ? 'glass-btn-base glass-btn-tone-info'
                    : 'text-[var(--glass-text-secondary)] hover:bg-[var(--glass-bg-muted)]'
                    }`}
                >
                  <AppIcon name="bookOpen" className="w-5 h-5" />
                  <span className="font-medium">{t('promptCenter.navLabel')}</span>
                </button>
              </nav>
              {/* 退出登录 */}
              <button
